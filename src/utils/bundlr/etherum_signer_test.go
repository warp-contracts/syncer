...
func TestEtherumSignerTestSuite(t *testing.T) {
	suite.Run(t, new(EtherumSignerTestSuite))
}

type EtherumSignerTestSuite struct {
	suite.Suite
}

func (s *EtherumSignerTestSuite) SetupSuite() {

}

func (s *EtherumSignerTestSuite) TearDownSuite() {
}

func (s *EtherumSignerTestSuite) TestCreation() {
	signer, err := NewEtherumSigner(ETHERUM_PRIVATE_KEY)
	require.Nil(s.T(), err)
...
func (s *EtherumSignerTestSuite) TestSignAndVerify() {
	signer, err := NewEtherumSigner(ETHERUM_PRIVATE_KEY)
	require.Nil(s.T(), err)
	require.NotNil(s.T(), signer)
