...
func (self *Collector) Collect(ch chan<- prometheus.Metric) {
	...
	ch <- prometheus.MustNewConstMetric(self.SyncerCurrentHeight, prometheus.GaugeValue, float64(self.monitor.Report.BlockDownloader.State.SyncerCurrentHeight.Load()))
	...
	ch <- prometheus.MustNewConstMetric(self.TxDownloadErrors, prometheus.CounterValue, float64(self.monitor.Report.TransactionDownloader.Errors.TxDownloadErrors.Load()))
