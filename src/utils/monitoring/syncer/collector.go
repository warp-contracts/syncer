...
func (self *Collector) Collect(ch chan<- prometheus.Metric) {
	...
	ch <- prometheus.MustNewConstMetric(self.SyncerBlocksBehind, prometheus.GaugeValue, float64(self.monitor.Report.BlockDownloader.State.BlocksBehind.Load()))
	ch <- prometheus.MustNewConstMetric(self.SyncerCurrentHeight, prometheus.GaugeValue, float64(self.monitor.Report.BlockDownloader.State.CurrentHeight.Load()))
	...
	ch <- prometheus.MustNewConstMetric(self.TxDownloadErrors, prometheus.CounterValue, float64(self.monitor.Report.BlockDownloader.Errors.BlockDownloadErrors.Load()))
