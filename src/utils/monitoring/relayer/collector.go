...
func (self *Collector) Collect(ch chan<- prometheus.Metric) {
	...
	ch <- prometheus.MustNewConstMetric(self.ArwaeveFinishedHeight, prometheus.CounterValue, float64(self.monitor.Report.Relayer.State.ArwaeveFinishedHeight.Load()))
	ch <- prometheus.MustNewConstMetric(self.SequencerFinishedHeight, prometheus.CounterValue, float64(self.monitor.Report.Relayer.State.SequencerFinishedHeight.Load()))
