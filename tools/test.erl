-module(test).
-export([start/0, testint/0, testbin/0, testlist/0, test_bin_real/0, test_range/0, test_io_list/0, test_raw/0, test_bigger/0]).
...
testint() ->
	io:format("<<~s>>~n", [[io_lib:format("~2.16.0B",[X]) || <<X:8>> <= encode_int(0, 16) ]]).

testbin() ->
	io:format("<<~s>>~n", [[io_lib:format("~2.16.0B",[X]) || <<X:8>> <= encode_bin(<<9,7>>, 16) ]]).

testlist() ->
	io:format("<<~s>>~n", [[io_lib:format("~2.16.0B",[X]) || <<X:8>> <= encode_bin_list([], 8, 16) ]]).
...
test_raw() ->
	io:format("<<~s>>~n", [[io_lib:format("~2.16.0B",[X]) || <<X:8>> <= <<1234:256>> ]]).
